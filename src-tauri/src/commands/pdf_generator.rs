use tauri::command;
use oxidize_pdf::{Document, Page as OxidizePage, Font, Image};
use crate::models::pdf_structure::{PdfStructure, Page as PdfPage, PdfMetadata, ImageFormat};
...
fn create_page_from_structure(page_data: &PdfPage, jp_font_embedded: bool) -> Result<OxidizePage, String> {
    // ページサイズの検証
    if page_data.width.is_nan() || page_data.width.is_infinite() ||
       page_data.height.is_nan() || page_data.height.is_infinite() {
        return Err(format!("無効なページサイズ（NaNまたはInfinity）: {:.2}x{:.2}", page_data.width, page_data.height));
    }
    if page_data.width <= 0.0 || page_data.height <= 0.0 {
        return Err(format!("無効なページサイズ: {:.2}x{:.2}", page_data.width, page_data.height));
    }
    ...
    for text_block in &page_data.text_blocks {
        ...
        // フォントサイズの検証
        if text_block.font_size.is_nan() || text_block.font_size.is_infinite() || text_block.font_size <= 0.0 {
            log::warn!("無効なフォントサイズ: {}, スキップします", text_block.font_size);
            continue;
        }
        ...
        // 座標の検証（警告のみ、エラーにはしない）
        if text_block.x.is_nan() || text_block.x.is_infinite() ||
           y_position.is_nan() || y_position.is_infinite() {
            log::warn!("テキストブロックの座標が無効です（NaNまたはInfinity）: x={}, y={}", text_block.x, y_position);
            continue;
        }
        if text_block.x < 0.0 || y_position < 0.0 {
            log::warn!("テキストブロックの座標が範囲外です: x={}, y={}", text_block.x, y_position);
        }
    ...
    for (index, image) in page_data.images.iter().enumerate() {
        ...
        // 画像サイズの検証
        if image.width.is_nan() || image.width.is_infinite() ||
           image.height.is_nan() || image.height.is_infinite() {
            log::warn!("無効な画像サイズ（NaNまたはInfinity）: {:.2}x{:.2}, スキップします", image.width, image.height);
            continue;
        }
        if image.width <= 0.0 || image.height <= 0.0 {
            log::warn!("無効な画像サイズ: {:.2}x{:.2}, スキップします", image.width, image.height);
            continue;
        }
        ...
        // 座標の検証（警告のみ、エラーにはしない）
        if image.x.is_nan() || image.x.is_infinite() ||
           y_position.is_nan() || y_position.is_infinite() {
            log::warn!("画像の座標が無効です（NaNまたはInfinity）: x={}, y={}", image.x, y_position);
            continue;
        }
